...
with open('causal_timo.pkl', 'wb') as f:
...
torch.save(model_state_dict, 'causal_timo.pth')
...
print("Relative Error Test for displacement: ", relative_error_test.detach().numpy()*100, "%")
...
print("Relative Error Test for pressure: ", relative_error_test.detach().numpy()*100, "%")

# for data generation

x_test = 8*pi*torch.rand(10000).reshape(-1,1)
t_test = torch.rand(10000)
test = torch.cat([x_test, t_test],1)
...
np.savetxt('data_u.csv' , [p for p in data.detach().numpy()],  delimiter=',' , fmt='%s')

data = torch.cat([x_test, t_test, p_test_pred], 1)
np.savetxt('data_p.csv' , [p for p in data.detach().numpy()],  delimiter=',' , fmt='%s')

u_test1 = exact_solution_u(x_test, t_test).reshape(-1,1)
p_test1 = exact_solution_p(x_test, t_test).reshape(-1,1)


# Compute the relative L2 error norm (generalization error)
relative_error_test = torch.mean((u_test_pred - u_test1)**2)/torch.mean(u_test**2)
#relative_error_test = torch.max(torch.abs(u_test_pred -u_test))/torch.max(torch.abs(u_test))
print("Relative Error Test for displacement (r): ", relative_error_test.detach().numpy()*100, "%")


relative_error_test = torch.mean((p_test_pred - p_test1)**2)/torch.mean(p_test**2)
print("Relative Error Test for pressure (r): ", relative_error_test.detach().numpy()*100, "%")
