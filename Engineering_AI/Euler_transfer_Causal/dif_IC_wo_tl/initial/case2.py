...
def exact_solution(x, t):
    return 2*torch.sin(x)*torch.exp(t)
        ...
        for j, (initial, u_initial, u_initial_t, bd_left, bd_right, ubl, ubr) in enumerate(training_set):
            def closure():
                ...
                pde_single_column = (u_tt.reshape(-1, ) + u_xxxx.reshape(-1, ) + \
                                     u_hat.reshape(-1, ) - 6*torch.sin(interior[:,0])*torch.exp(interior[:,1])) ** 2
...
with open('p_ic_n_2.pkl', 'wb') as f:
...
torch.save(model_state_dict, 'p_ic_n_2.pth')
