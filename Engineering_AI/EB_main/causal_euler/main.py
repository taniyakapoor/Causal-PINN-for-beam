...
plt.ylabel("u")
plt.legend()
plt.savefig("causal_euler1.pdf")

x_test = 8*pi*torch.rand(10000).reshape(-1,1)
t_test = torch.rand(10000).reshape(-1,1)
test = torch.cat([x_test, t_test],1)
u_test = exact_solution(x_test, t_test).reshape(-1,1)
...
np.savetxt('data_prob1.csv' , [p for p in data.detach().numpy()],  delimiter=',' , fmt='%s')

# Compute the relative L2 error norm (generalization error)
relative_error_test = torch.mean((u_test_pred - u_test)**2)/torch.mean(u_test**2)
print("Relative Error Test for random points: ", relative_error_test.detach().numpy()*100, "%")
