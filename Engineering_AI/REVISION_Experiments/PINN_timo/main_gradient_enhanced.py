    ...
    for epoch in range(num_epochs):
        ...
        for j, (initial, w_initial, w_initial_t, bd_left, bd_right) in enumerate(training_set):
            def closure():
                ...
                u_tt = grad_u_hat_tt[:, 0].reshape(-1, 1)
                ...
                grad_u_hat_tt = torch.autograd.grad(u_tt, interior, grad_outputs=inputs, create_graph=True)[0]
                u_ttx = grad_u_hat_tt[:, 0]
                ...
                grad_p_hat_ttt = torch.autograd.grad(p_tt, interior, grad_outputs=inputs, create_graph=True)[0]
                p_ttx = grad_p_hat_ttt[:, 0].reshape(-1, 1)
                ...
                loss_pde_enhanced = (u_ttx.reshape(-1, ) - u_xxx.reshape(-1, ) + u_x.reshape(-1, ) - p_xx.reshape(-1, )) ** 2
                
                loss_pde_enhanced1 = (p_ttx.reshape(-1, ) - u_xx.reshape(-1, ) - p_xxx.reshape(-1, ) + p_x.reshape(-1, )) ** 2
                ...
                loss = loss1 + loss2 +  torch.mean(loss_pde_enhanced1) + torch.mean(loss_pde_enhanced)
...
# with open('PINN_tim_gradient_new.pkl', 'wb') as f:
...
# torch.save(model_state_dict, 'PINN_tim_gradient_new.pth')


# # Load the history from the pickle file
# with open('PINN_tim_gradient_new.pkl', 'rb') as f:
...
# model_state_dict = torch.load('PINN_tim_gradient_new.pth')
...
plt.savefig('PINN_timo_disp_gradient_new.png', dpi=500, bbox_inches="tight", format='png')
...
# plt.xticks(fontsize=font_size, fontproperties=ticks_font)
# plt.yticks(fontsize=font_size, fontproperties=ticks_font)
plt.savefig('PINN_timo_rotation_gradient_new.png', dpi=500, bbox_inches="tight", format='png')
