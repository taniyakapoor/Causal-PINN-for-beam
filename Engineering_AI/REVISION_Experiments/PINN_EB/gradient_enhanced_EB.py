...
#                 # gradient enhanced with respect to t:
                
#                 grad_u_tt = torch.autograd.grad(u_tt, interior, grad_outputs=inputs, create_graph=True)[0]
#                 u_ttt = grad_u_tt[:, 1].reshape(-1, )
                
#                 grad_u_xxxx = torch.autograd.grad(u_xxxx, interior, grad_outputs=inputs, create_graph=True)[0]
#                 u_xxxxt = grad_u_xxxx[:, 1].reshape(-1, )
                
#                 loss_e_t = u_ttt + u_xxxxt + u_t + (2-pi**2)*pi*torch.sin(interior[:, 0])*torch.sin(interior[:, 1]*pi)
                
                
                # gradient enhanced with respect to x:
                
                
                grad_u_tt = torch.autograd.grad(u_tt, interior, grad_outputs=inputs, create_graph=True)[0]
                u_ttx = grad_u_tt[:, 0].reshape(-1, )
                
                grad_u_xxxx = torch.autograd.grad(u_xxxx, interior, grad_outputs=inputs, create_graph=True)[0]
                u_xxxxx = grad_u_xxxx[:, 0].reshape(-1, )
                
                loss_e_x = u_ttx + u_xxxxx + u_x - (2-pi**2)*torch.cos(interior[:, 0])*torch.cos(interior[:, 1]*pi)
                ...
                loss = loss_ic + loss_pde + loss_left_b + loss_right_b + torch.mean((loss_e_x)**p) 
...
with open('PINN_EB_gradient.pkl', 'wb') as f:
...
torch.save(model_state_dict, 'PINN_EB_gradient.pth')
