...
wt_7 = torch.zeros(10000, 1)
wt_8 = torch.zeros(10000, 1)
wt_9 = torch.zeros(10000, 1)
wt_10 = torch.zeros(10000, 1)
#wt_6 = torch.zeros(10000, 1)
...
error_7 = torch.zeros(10000, 1)
error_8 = torch.zeros(10000, 1)
error_9 = torch.zeros(10000, 1)
error_10 = torch.zeros(10000, 1)
#error_6 = torch.zeros(10000, 1)
    ...
    for epoch in range(num_epochs):
        ...
        for j, (initial, u_initial, u_initial_t, bd_left, bd_right, ubl, ubr) in enumerate(training_set):
            def closure():
                ...
                weight1 = weighted_loss[6]
                weight2 = weighted_loss[29]
                weight3 = weighted_loss[30]
                weight4 = weighted_loss[41]
                weight5 = weighted_loss[45]
                weight6 = weighted_loss[53]
                weight7 = weighted_loss[62]
                weight8 = weighted_loss[69]
                weight9= weighted_loss[78]
                weight10 = weighted_loss[98]
...
#                 loss_6[epoch, :] = loss6
                ...
                with torch.no_grad():
                    ...
                    # Test data
                    t_test7 = 0.62*torch.ones((100,1))
                    test7 = torch.cat([x_test1, t_test7],1).to(device)
                    u_test7 = exact_solution(x_test1, t_test7).reshape(-1,1)
                    #my_network = my_network.to(device)
                    u_test_pred7 = my_network(test7).reshape(-1,1).to('cpu')
                    
                    # Compute the relative L2 error norm (generalization error)
                    error_7[epoch, :] = torch.mean((u_test_pred7 - u_test7)**2)/torch.mean(u_test7**2)
                    
                     # Test data
                    t_test8 = 0.69*torch.ones((100,1))
                    test8 = torch.cat([x_test1, t_test8],1).to(device)
                    u_test8 = exact_solution(x_test1, t_test8).reshape(-1,1)
                    #my_network = my_network.to(device)
                    u_test_pred8 = my_network(test8).reshape(-1,1).to('cpu')
                    
                    # Compute the relative L2 error norm (generalization error)
                    error_8[epoch, :] = torch.mean((u_test_pred8 - u_test8)**2)/torch.mean(u_test8**2)
                    
                    t_test9= 0.78*torch.ones((100,1))
                    test9 = torch.cat([x_test1, t_test9],1).to(device)
                    u_test9 = exact_solution(x_test1, t_test9).reshape(-1,1)
                    #my_network = my_network.to(device)
                    u_test_pred9 = my_network(test9).reshape(-1,1).to('cpu')
                    
                    # Compute the relative L2 error norm (generalization error)
                    error_9[epoch, :] = torch.mean((u_test_pred9 - u_test9)**2)/torch.mean(u_test9**2)
                    
                    
                    t_test10=0.98*torch.ones((100,1))
                    test10= torch.cat([x_test1, t_test10],1).to(device)
                    u_test10 = exact_solution(x_test1, t_test10).reshape(-1,1)
                    #my_network = my_network.to(device)
                    u_test_pred10 = my_network(test10).reshape(-1,1).to('cpu')
                    
                    # Compute the relative L2 error norm (generalization error)
                    error_10[epoch, :] = torch.mean((u_test_pred10 - u_test10)**2)/torch.mean(u_test10**2)
